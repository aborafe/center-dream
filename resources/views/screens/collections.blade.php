...
<section class="screen" aria-labelledby="collection-title">
    ...
    <form class="panel collection-form" method="POST" action="{{ route('collections.store') }}">
        ...
        <div class="form-grid">
            <div class="field-group"><label for="enrollment_id">اشتراك الطالب</label><select id="enrollment_id" name="enrollment_id" required>@forelse ($enrollments as $enrollment)<option value="{{ $enrollment->id }}" @selected((int) old('enrollment_id', request()->integer('enrollment')) === $enrollment->id)>{{ $enrollment->student->name }} — {{ $enrollment->subject->name }} · المتبقي {{ number_format((float) $enrollment->fee - (float) $enrollment->discount_amount - (float) $enrollment->payments->sum('amount'), 2) }} ج.م</option>@empty<option value="">لا توجد اشتراكات لها رصيد</option>@endforelse</select></div>
