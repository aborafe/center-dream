...
<section class="screen" aria-labelledby="payouts-title">
    <x-page-header title="صرف مستحقات المدرسين" subtitle="سجّل الصرف مع الفترة وطريقة الدفع؛ يظهر لاحقًا في التقارير المالية." />
    ...
    @if ($teachers->isNotEmpty())
        <form class="panel collection-form" method="POST" action="{{ route('teacher-payouts.store') }}">
            ...
            <div class="form-grid">
                ...
                <div class="field-group"><label for="payout-subject">من محفظة مادة (اختياري)</label><select id="payout-subject" name="subject_id"><option value="">صرف عام من محفظة المدرس</option>
                    @foreach ($teachers as $teacher)
                        @foreach ($teacher->subjects as $subject)
                            <option value="{{ $subject->id }}" @selected(old('subject_id') == $subject->id)>{{ $teacher->name }} — {{ $subject->name }}</option>
                        @endforeach
                    @endforeach
                </select></div>
                ...
                <x-input label="بداية الفترة" name="period_from" type="date" value="{{ old('period_from', now()->startOfMonth()->format('Y-m-d')) }}" autocomplete="off" required />
                <x-input label="نهاية الفترة" name="period_to" type="date" value="{{ old('period_to', now()->format('Y-m-d')) }}" autocomplete="off" required />
            ...
            @foreach (['teacher_id', 'amount', 'period_from', 'period_to', 'method', 'note'] as $field) @error($field)<p class="form-message" role="alert">{{ $message }}</p>@enderror @endforeach
    ...
    <section class="panel structured-list">
        <div class="panel-heading"><h2>عمليات الصرف</h2><span class="muted">مرتبطة بالمدرس والمنفذ والفترة.</span></div>
        <div class="table-wrap"><table><thead><tr><th>المدرس</th><th>المادة</th><th>المبلغ</th><th>الفترة</th><th>الطريقة</th><th>المنفذ</th><th>التاريخ</th></tr></thead><tbody>@forelse ($payouts as $payout)<tr><td class="strong">{{ $payout->teacher->name }}</td><td>{{ $payout->subject?->name ?? 'صرف عام' }}</td><td class="amount-due">{{ number_format((float) $payout->amount, 2) }} ج.م</td><td>{{ $payout->period_from->translatedFormat('j F') }} — {{ $payout->period_to->translatedFormat('j F Y') }}</td><td>{{ ['cash' => 'نقدي', 'transfer' => 'تحويل', 'wallet' => 'محفظة'][$payout->method] }}</td><td>{{ $payout->payer->name }}</td><td>{{ $payout->paid_at->translatedFormat('j F Y') }}</td></tr>@empty<tr><td colspan="7" class="muted">لا توجد عمليات صرف مسجلة حتى الآن.</td></tr>@endforelse</tbody></table></div>
    </section>
</section>
@endsection
