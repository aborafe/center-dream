...
<section class="screen reports-screen" id="reports" aria-labelledby="reports-title">
    ...
    <form class="panel report-filters" method="GET" action="{{ route('reports.index') }}" data-date-range-picker>
        ...
        <div class="field-group"><label for="report-teacher">المدرس</label><select id="report-teacher" name="teacher_id"><option value="">كل المدرسين</option>@foreach ($teachers as $teacher)<option value="{{ $teacher->id }}" @selected(($filters['teacher_id'] ?? null) == $teacher->id)>{{ $teacher->name }}{{ $teacher->is_active ? '' : ' — موقوف' }}</option>@endforeach</select></div>
        ...
        <div class="report-export-actions" aria-label="تصدير ومشاركة التقرير"><a class="outline-button" href="{{ route('reports.preview', $filters) }}" target="_blank" rel="noopener">معاينة وتصدير PDF</a><a class="outline-button" href="{{ route('reports.whatsapp', $filters) }}">إرسال عبر واتساب</a></div>
    </form>
