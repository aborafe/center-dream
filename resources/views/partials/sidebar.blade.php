...
<aside class="sidebar center-sidebar" id="app-sidebar">
    ...
    <nav class="side-nav center-nav" aria-label="التنقل الرئيسي">
        ...
        @if ($currentUser->hasPermission('collections'))<a @class(['nav-item', 'is-active' => request()->routeIs('daily-cashbook.*')]) href="{{ route('daily-cashbook.index') }}"><span class="nav-icon"><x-icon name="wallet" /></span><span class="nav-label">إيراد ومصروف اليوم</span></a>@endif
    ...
    <div class="profile-row center-profile-row">
        ...
        <form class="sidebar-logout-form" method="POST" action="{{ route('logout') }}">@csrf<button class="sidebar-logout" type="submit" aria-label="تسجيل الخروج" title="تسجيل الخروج"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 3.75h9.5v16.5H5zM14.5 12H3.5m0 0 3-3m-3 3 3 3"/></svg></button></form>
    </div>
</aside>
